typeRus = {
    'Attaker': 'Атакер', 'Defender': 'Защитник', 'Shooter': 'Стрелок'
}
tupeNum = {
    0: 'Attaker', 1: 'Defender', 2: 'Shooter'
}
typeHP = {
    'Attaker': 15, 'Defender': 50, 'Shooter': 10
}
typeDamag = {
    'Attaker': 5, 'Defender': 2, 'Shooter': 30
}
typeKD = {
    'Attaker': 10, 'Defender': 10, 'Shooter': 30
}
typeDist = {
    'Attaker': 30, 'Defender': 50, 'Shooter': 400
}
typeSpeed = {
    'Attaker': 3, 'Defender': 2.5, 'Shooter': 2
}
typeCost = {
    'Attaker': 110, 'Defender': 150, 'Shooter': 200
}
typeColors = {
    'Attaker': (255, 165, 0),
    'Defender': (0, 128, 128),
    'Shooter': (0, 255, 0)
}
typeImagePath = {
    'AttakerN': r'data/atacker_n.png',
    'DefenderN': r'data/defender_n.png',
    'ShooterN': r'data/shooter_n.png',
    'AttakerE': r'data/atacker_e.png',
    'DefenderE': r'data/defender_e.png',
    'ShooterE': r'data/shooter_e.png'
}
gamebackground = r'data/game_background.png'
menubackground = r'data/background_menu.png'
levelPath = r'data/level'
